...
from tests.e2e_entity.haeo_02_net_zero_homebattery_primary_ev_surplus.scenario_steps import (
    build_harness,
    run_steps,
)
...
def test_haeo_net_zero_internal_plan_homebattery_primary_ev_adjustable(project_root):
    ...
    steps = [
        {
            'at_s': 0,
            'note': 'quarter start: internal HAEO plan selects HOME_BATTERY primary and EV surplus',
        ...
        {
            'at_s': 30,
            'note': 'next policy cycle: EV surplus state is active and EV current is capped by HAEO limit',
            'set': runtime_inputs_for_net_zero_intent(
                E,
                rpnz_w=6000.0,
                required_power_consumption_kw=4.5,
