...
Effective primary poistetaan surplus-poolista saman tickin ajaksi. Surplus allocator ohittaa `activation_allowed=false`-kandidaatin ja vapauttaa aktiivisen unavailable-laitteen. Lifecycle-release muuttaa vain eligibilityn; EV:n final surplus-DevicePolicy aktivoituu vasta persisted active-state/dispatch-ketjun jälkeen.


Surplus-state säilyttää aktiiviset device-ID:t activation orderissa. Kun aktiivisia laitteita on `n > 1`, allocator arvioi vain uusimman n−1-portaan. Release syntyy, kun negatiivisen RPC:n suuruus on vähintään `releasable_power_w - max(100 W, 0.05 × releasable_power_w)`. Päätös vapauttaa yhden laitteen ja kirjoittaa uuden measurement-settle-freezen. Ensimmäisenä aktivoitu anchor arvioidaan vasta yksin jäätyään vanhalla RPNZ-deadband-säännöllä.

`releasable_power_w` on fixed-laitteella nimellisteho. Säädettävällä laitteella se on nykyinen positiivinen control target, fallbackina konfiguroitu surplus-target. Diagnostics raportoi activation orderin, release-moodin, tehon, marginaalin, kynnyksen ja excess-tehon.
...
Persistoitava continuity sisältää vain seuraavien ajojen päätöksentekoon tarvittavaa statea. EV:n `low_pv_cycles` saturoituu HARD_OFF-kynnykseen. HARD_OFF-release-counter etenee vain, kun PV on palautunut kynnykselle tai sen yli eikä activation block ole aktiivinen. RPC ja surplus activation threshold kuuluvat allocatoriin eivätkä lifecycle-releaseen.
