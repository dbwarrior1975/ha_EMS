...
5. Device-kohtainen surplus-kandidaatti- ja lifecycle-malli
...
## 5) Device-kohtainen surplus-kandidaattimalli

```mermaid
stateDiagram-v2
    [*] --> INELIGIBLE
    INELIGIBLE --> ELIGIBLE: can_absorb && surplus_allowed && runtime_available
    ELIGIBLE --> ACTIVE: strict-priority allocator ACTIVATE device_id
    ACTIVE --> ELIGIBLE: RELEASE device_id
    ELIGIBLE --> BLOCKED: lifecycle activation_allowed=false
    BLOCKED --> ELIGIBLE: consecutive recovery cycles reached
    ACTIVE --> INELIGIBLE: policy/capability withdrawn -> release path
```
