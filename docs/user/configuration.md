...
Kun EV ei ole aktiivisessa SURPLUS-listassa mutta sen toteutunut ohjausteho on konfiguroidulla `min_absorb_w`-tasolla, EMS tulkitsee laitteen min-hold-tilaan. Tällöin seuraavan SURPLUS-portaan RPC-kynnys kuvaa vain käytettävissä olevaa lisäkulutusta:

```text
activation_threshold_w = max_absorb_w - min_absorb_w
```

Esimerkiksi `min_absorb_w = 1840 W` ja `max_absorb_w = 7400 W` tuottavat `5560 W` aktivointikynnyksen. Aktivoinnin final target on silti absoluuttinen `7400 W`. Tunnistus sallii mittaukselle puolen `step_w`-arvon toleranssin. Kun aktiivinen EV vapautetaan takaisin min-holdiin, myös n−1-releasen `releasable_power_w` lasketaan poistuvasta lisätehosta, ei koko absoluuttisesta EV-tehosta.
